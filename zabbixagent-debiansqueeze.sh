...
# Script libre: GPLv3
#
# Syntaxe: # sudo ./zabbixagent-debiansqueeze.sh
...
if [ "$(id -u)" != "0" ]; then
	echo "Le script doit etre execute en tant que root."
  	echo "Syntaxe: su - ./zabbixagent-debiansqueeze.sh"
  	exit 1
fi
...
case "$1" in
    installe)
		...
		echo "Telechargement du script zabbix_agentd 'source Zabbix SIA'"
			wget --no-check-certificate https://github.com/downloads/lbtm/zabbixagentinstalle/zabbix-agent
			chmod a+x zabbix-agent
